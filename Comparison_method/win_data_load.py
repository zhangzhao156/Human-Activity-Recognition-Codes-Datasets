...
def load_WISDMar(subseq):
    ...
    reshaped_labels = np.asarray(pd.get_dummies(labels), dtype = np.float32)
    act_classes = reshaped_labels.shape[1]
    print('label shape')
...
def load_UCI_HAR(subseq):
    ...
    reshaped_labels = np.asarray(labels, dtype=np.float32).reshape(-1,N_TIME_STEPS, act_classes)
    print('label shape')
    print(reshaped_labels.shape)
...
def load_UCI_HAPT(subseq):
    ...
    for i in range(0, len(np_df) - N_TIME_STEPS, step):
        tmp = norm_np_df[i: i + N_TIME_STEPS, :]
        segments.append(tmp)
        labels.append(stats.mode(label[i: i + N_TIME_STEPS])[0][0])
    reshaped_segments = np.asarray(segments, dtype=np.float32).reshape(-1, N_TIME_STEPS, N_FEATURES)
    print('\ndata shape')
    print(reshaped_segments.shape)
    reshaped_labels = np.asarray(pd.get_dummies(labels), dtype=np.float32)
